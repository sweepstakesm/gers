var Poll = function(id, random) {
    ...
    var self = this,
        ...
        votingAllowed = !1,
        displayChart = 0,
        chartData, chartDatasetsOptions, totalVotes = 0,
        showChart = !1,
        instance = null,
        optionIndex = 0,
        usePercentages = !1;
    ...
    function getChartData() {
        $.post(ajaxurl, {
            ...
            chartData = $.parseJSON(data), chartType = chartData.chart_type;
            for (var i = 0; i < chartData.totals.length; i++)
                chartData.totals[i] = Math.ceil(parseInt(chartData.totals[i]));
            self.usePercentages = (chartData.percentage) ? !0 : !1;
    ...
    function loadBootstrap() {
        ...
        var bsChartContainer = $('<div/>').attr("class", "bs-chart-container").css("display", "block").css("width", "100%");
        for (var i = 0; i < chartDatasetsOptions.length; i++) {
            ...
            var progressBar = $('<div/>').attr("class", "weblator-poll-progress-bar").attr("role", "progressbar").attr("aria-valuenow", p).attr("aria-valuemin", 0).css({
            ...
            var span = $('<span/>').attr("class", "").html((chartData.percentage) ? chartDatasetsOptions[i].value + "%" : chartDatasetsOptions[i].value);
            if (p > 0)
    ...
    function submitVote() {
        if (!container.find(".weblator-poll-vote ul li input[type='radio']:checked").length) {
            ...
            $.post(ajaxurl, {
                ...
                option: option,
                poll_id: id,
                '_wpnonce': container.attr('data-_wpnonce')
            }, function(response) {
                container.find("input[name='weblator-chart-options']:checked").prop('checked', !1);
                canUserVote();
                if (votingAllowed) {
